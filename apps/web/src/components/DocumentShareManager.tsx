        ...
        {!isXlsx ? (
          ...
          <section className="grid gap-4 rounded-lg border p-4">
            ...
            <label className="grid gap-1.5 text-sm"><span>收件者</span>
              ...
              {users.data && !recipient && <div className="overflow-hidden rounded-md border">{users.data.map((user) => <button key={user.id} type="button" className="block w-full border-b px-3 py-2 text-left text-sm last:border-0 hover:bg-muted" onClick={() => { setRecipient(user); setSearchQuery(''); setSearchInput(''); }}>{user.displayName} <span className="text-muted-foreground">{user.email}</span></button>)}</div>}
            ...
            <div className="flex justify-end"><Button disabled={!recipient || create.isPending} onClick={() => create.mutate()}>{create.isPending ? '建立中…' : '建立分享'}</Button></div>
          </section>

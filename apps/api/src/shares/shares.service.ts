...
export class SharesService {
  ...
  private get shareRepository() {
    return (this.prisma as unknown as { documentShare: {
  ...
  private async assertActiveShare(userId: string, shareId: string): Promise<DocumentShare> {
    const share = await this.shareRepository.findFirst({ where: { id: shareId, recipientId: userId } });
  ...
  private async createMaskedCopy(documentId: string, rules: MaskRuleDto[]): Promise<{ key: string; sourceVersionId: string }> {
    const document = await this.prisma.document.findUnique({ where: { id: documentId }, include: { currentVersion: true } });
    if (!document?.currentVersion) throw new NotFoundException('Document or current version not found');
    if (document.currentVersion.mimeType !== XLSX_MIME) {
      throw new BadRequestException('Column masking is currently supported only for .xlsx documents');
    }
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(await this.streamToBuffer(await this.storage.getObjectStream(document.currentVersion.objectKey)) as never);
    for (const rule of rules) {
      ...
      let column = -1;
      headerRow.eachCell((cell, columnNumber) => { if (String(cell.value ?? '') === rule.header) column = columnNumber; });
      if (column < 1) throw new BadRequestException(`Column header not found: ${rule.sheetName}.${rule.header}`);
      worksheet.eachRow((row, rowNumber) => {
        if (rowNumber > 1) row.getCell(column).value = this.maskValue(row.getCell(column).value, rule.mode);
      });
    }
  ...
  async create(user: AuthenticatedUser, documentId: string, dto: CreateDocumentShareDto, ipAddress: string | null) {
    ...
    if (rules.length > 0 && await this.acl.resolveEffectiveLevel({ id: recipient.id, roles: [] }, 'document', documentId)) {
      throw new BadRequestException('Recipient already has document access; revoke that access before creating a masked share');
    }
    ...
    const share = await this.shareRepository.create({
      data: {
        documentId, recipientId: recipient.id, createdBy: user.id, accessLevel: dto.accessLevel,
        expiresAt: new Date(Date.now() + dto.durationHours * 60 * 60 * 1000), maskRules: rules,
        maskedObjectKey: masked?.key, sourceVersionId: masked?.sourceVersionId,
      },
    });
    await this.audit.recordSafely({ actorId: user.id, action: 'document_share_create' as never, resourceType: 'document', resourceId: documentId, ipAddress, details: { shareId: share.id, recipientId: recipient.id, accessLevel: dto.accessLevel, durationHours: String(dto.durationHours), masked: String(rules.length > 0) } });
    return share;
  }
  ...
  async listReceived(user: AuthenticatedUser) {
    return this.shareRepository.findMany({ where: { recipientId: user.id, revokedAt: null, expiresAt: { gt: new Date() } }, include: { document: { select: { name: true } } }, orderBy: { expiresAt: 'asc' } });
  }

  async update(user: AuthenticatedUser, shareId: string, dto: UpdateDocumentShareDto, ipAddress: string | null) {
    ...
    const updated = await this.shareRepository.update({ where: { id: shareId }, data: { ...(dto.accessLevel && { accessLevel: dto.accessLevel }), ...(dto.durationHours && { expiresAt: new Date(Date.now() + dto.durationHours * 60 * 60 * 1000) }) } });
    await this.audit.recordSafely({ actorId: user.id, action: 'document_share_update' as never, resourceType: 'document', resourceId: share.documentId, ipAddress, details: { shareId, accessLevel: dto.accessLevel ?? '', durationHours: String(dto.durationHours ?? '') } });
    return updated;
  }
  ...
  async revoke(user: AuthenticatedUser, shareId: string, ipAddress: string | null) {
    ...
    await this.audit.recordSafely({ actorId: user.id, action: 'document_share_revoke' as never, resourceType: 'document', resourceId: share.documentId, ipAddress, details: { shareId } });
  }
  ...
  async getContent(user: DownloadingUser, shareId: string, ipAddress: string | null) {
    ...
    const document = await this.prisma.document.findUnique({ where: { id: share.documentId }, include: { currentVersion: true } });
    if (!document?.currentVersion || document.deletedAt) throw new NotFoundException('Document not found');
    const key = share.maskedObjectKey ?? document.currentVersion.objectKey;
    await this.audit.recordSafely({ actorId: user.id, action: 'document_share_access' as never, resourceType: 'document', resourceId: document.id, ipAddress, details: { shareId, access: 'download' } });
    return { stream: await this.storage.getObjectStream(key), mimeType: document.currentVersion.mimeType, fileName: document.name, share };
  }
  ...
  async getEditorConfig(user: DownloadingUser, shareId: string, ipAddress: string | null) {
    ...
    const config = { documentType: 'spreadsheet', document: { fileType: 'xlsx', key: `${share.id}-${share.updatedAt.getTime()}`, title: document.name, url: `${baseUrl}/shares/${share.id}/content?editorToken=${token}` }, editorConfig: { mode: share.accessLevel === 'edit' ? 'edit' : 'view', callbackUrl: `${baseUrl}/shares/${share.id}/onlyoffice/callback?editorToken=${token}`, user: { id: user.id, name: user.email } } };
  ...
  private createEditorToken(shareId: string, recipientId: string) {
    const secret = process.env.ONLYOFFICE_JWT_SECRET;
    if (!secret) throw new BadRequestException('OnlyOffice JWT is not configured');
    const payload = Buffer.from(JSON.stringify({ shareId, recipientId, exp: Math.floor(Date.now() / 1000) + 300 })).toString('base64url');
  ...
  async getEditorContent(shareId: string, editorToken: string) {
    ...
    const document = await this.prisma.document.findUnique({ where: { id: share.documentId }, include: { currentVersion: true } });
    if (!document?.currentVersion || document.deletedAt) throw new NotFoundException('Document not found');
    return { stream: await this.storage.getObjectStream(share.maskedObjectKey ?? document.currentVersion.objectKey), mimeType: document.currentVersion.mimeType, fileName: document.name };
  }

  async saveOnlyOfficeResult(shareId: string, editorToken: string, body: { status?: number; url?: string }) {
    const recipientId = this.verifyEditorToken(shareId, editorToken);
    const share = await this.assertActiveShare(recipientId, shareId);
  ...
  private verifyEditorToken(shareId: string, editorToken: string): string {
    ...
    if (signature.length !== expected.length || !timingSafeEqual(Buffer.from(signature), Buffer.from(expected))) throw new ForbiddenException('Invalid editor token');
    const decoded = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')) as { shareId?: string; recipientId?: string; exp?: number };
    if (decoded.shareId !== shareId || !decoded.recipientId || !decoded.exp || decoded.exp <= Math.floor(Date.now() / 1000)) throw new ForbiddenException('Editor token expired');
